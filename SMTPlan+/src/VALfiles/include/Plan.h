...
namespace VAL
{
    ...
    class EffectsRecord
    {
      ...
      public:
        void pushAdd(const SimpleProposition* p, const Action* act)
        {
            adds.push_back(p);
            if (act)
                responsibleForProps[p].insert(act);
        };
        ...
        void addFEffect(const FuncExp* fe, assign_op aop, FEScalar value, const Action* act)
        {
            ...
            if (act)
                responsibleForPNEs[fe].insert(act);
        };
    ...
    struct after
    {
        ...
        after(double t, double tol)
            : time(t), tolerance(tol){};
      ...
      public:
                               ...
                               <std::input_iterator_tag, const Happening*>
        {
          private:
            ...
            enum HappeningType { INVARIANT,
                CTS,
                REGULAR,
                END };  //The different types of happening to be executed in that order
          ...
          public:
            ...
            {
                for (map<const FuncExp*, ActiveFE*>::iterator j = ace.activeFEs.begin(); j != ace.activeFEs.end(); ++j) {  //cout << " deleting"<< *(j->second->fe) <<"\n";
                    delete j->second;
                };
