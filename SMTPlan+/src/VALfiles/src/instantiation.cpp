...
namespace Inst
{
    ...
    class ParameterDomains
    {
      ...
      public:
        ...
        ParameterDomains(const int& i)
            : domCount(i), domains(i, make_pair(true, set<int>())){};
        ParameterDomains(const ParameterDomains& d)
            : domCount(d.domCount), domains(d.domains){};
    ...
    class ParameterDomainConstraints : public VAL::VisitController
    {
      ...
      public:
        ParameterDomainConstraints(const VAL::operator_* const opIn, VAL::TypeChecker& tc)
            : op(opIn), varCount(op->parameters->size()), possibleParameterValues(varCount), parameterValuesToIndices(varCount), vars(varCount), equality(VAL::current_analysis->pred_tab.symbol_probe("="))
        {
        ...
        virtual void visit_simple_goal(VAL::simple_goal* s)
        {
            ...
            for (int gi = 0; groundItr != groundEnd; ++groundItr, ++gi) {
                ...
                if (keep) {
                    if (debug)
                        cout << "\t\tKeeping ground instance " << gi << "\n";
                    if (isect) {
                        for (int aff = 0; aff < affects; ++aff) {
                            if (scratch[aff] != -1) {
        ...
        virtual void visit_conj_goal(VAL::conj_goal* c)
        {
            ...
            if (!haveSpecial) {
                doUnion.pop_front();
                list<ParameterDomains>::iterator second = domainQueue.begin();
                ++second;
                for (int v = 0; v < varCount; ++v) {
