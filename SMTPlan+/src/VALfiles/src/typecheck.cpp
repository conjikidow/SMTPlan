 ...
 */
    ...
    struct matchFunc
    {
        ...
        matchFunc(const func_symbol* fs)
            : f(fs){};
    ...
    struct matchPred
    {
        ...
        matchPred(const pred_symbol* ps)
            : p(ps){};
    ...
    struct matchOp
    {
        ...
        matchOp(const operator_symbol* fs)
            : f(fs){};
 ...
 */
    ...
    bool TypeChecker::typecheckGoal(const goal* g)
    {
        ...
        if (const constraint_goal* cg = dynamic_cast<const constraint_goal*>(g)) {
            return (!cg->getRequirement() || typecheckGoal(cg->getRequirement())) && (!cg->getTrigger() || typecheckGoal(cg->getTrigger()));
        };
        return false;
    };
    ...
    bool TypeChecker::typecheckEffects(const effect_lists* es)
    {
        if (!isTyped)
            return true;
        return (es->add_effects.end() == std::find_if(es->add_effects.begin(), es->add_effects.end(), badchecker(this)))
    ...
    bool TypeChecker::typecheckDomain()
    {
        if (!isTyped)
            return true;
        return thea->the_domain->ops->end() == std::find_if(thea->the_domain->ops->begin(), thea->the_domain->ops->end(), badchecker(this))
               && thea->the_domain->drvs->end() == std::find_if(thea->the_domain->drvs->begin(), thea->the_domain->drvs->end(), badchecker(this))
