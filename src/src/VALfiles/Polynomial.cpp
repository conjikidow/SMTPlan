...
namespace VAL
{
    ...
    unsigned int descartesBound(const Polynomial& p)
    {
        ...
        if (noSignChanges == 0 && extraVal == 1) {
            ...
            if ((val0 < Polynomial::tooSmall && val0 > -Polynomial::tooSmall) || (val1 < Polynomial::tooSmall && val1 > -Polynomial::tooSmall)) {
                PolyRootError pre;
                throw pre;
            };
   ...
   */
    ...
    Intervals NumericalSolution::getIntervals(const Comparison* comp, const State* s, CoScalar t) const
    {
        ...
        if ((j->second > offSet && greater) || (j->second < offSet && !greater)) {
            ...
            double endPoint;
            if (j != points.end())
                endPoint = lastTime + (j->first - lastTime) * (fabs(lastPoint) / (fabs(lastPoint) + fabs(j->second)));
            else {
                endPoint = lastTime;
                strict = false;
            };
        ...
        while (j != points.end()) {  //point at j is not satisfied to begin with
            ...
            if (!startPointDefined) {
                ...
                if (startPointDefined)
                    startPt = make_pair(lastTime + (j->first - lastTime) * (fabs(lastPoint) / (fabs(lastPoint) + fabs(j->second))), !(strict));
            };
    ...
    bool NumericalSolution::checkInvariant(const Comparison* comp, const State* s, CoScalar t, bool rhsIntervalOpen) const
    {
        ...
        for (; j != points.end();) {
            ...
            if (j->first == t)
                break;
            if (((greater && (j->second - offSet < 0)) || (j->second - offSet <= 0 && !strict)) || ((!greater && (j->second - offSet > 0)) || (j->second - offSet >= 0 && !strict)))
                return false;
        };
        //check end point
        if (j != points.end()) {
            if (((greater && (j->second - offSet < 0)) || (j->second - offSet <= 0 && !strict && !rhsIntervalOpen)) || ((!greater && (j->second - offSet > 0)) || (j->second - offSet >= 0 && !strict && !rhsIntervalOpen)))
                return false;
        };
