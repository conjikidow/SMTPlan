            ...
            for (unsigned int i = 0; i < p->args->size(); ++i, ++ps) {
                ...
                for (vector<Property*>::const_iterator prp = ms.begin(); prp != ms.end(); ++prp) {
                    ...
                    if ((*prp)->isSingleValued()) {
                        cout << "Think I should allocate this to parameter " << (static_cast<const IDsymbol<var_symbol>*>(*ps)->getId()) << "\n";
                        gathered[(static_cast<const IDsymbol<var_symbol>*>(*ps)->getId())].push_back(make_pair(TPS(p->head)->property(i), p));
                        break;
                    };
                };
            };
