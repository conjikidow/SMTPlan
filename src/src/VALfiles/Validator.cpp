...
namespace VAL
{
    ...
    DerivationRules::DerivationRules(const derivations_list* d, const operator_list* o)
        : ops(o), derivPreds()
    {
        ...
        for (; disconts != discons.end(); ++disconts) {
            ...
            if ((conts != points.end()) && (conts->first < disconts->first)) {
                ...
                for (; conts->first <= disconts->first;) {
                    ...
                    if ((conts == points.end()) || (conts->first > disconts->first)) {
                        if (!ctsActivity)
                            drawLaTeXLine(prevTime, prevValue, disconts->first, disconts->second.first, maxTime);
                        else
                            drawLaTeXLine(nextTime, nextValue, disconts->first, disconts->second.first, maxTime);
    ...
    void changeActionTime(const plan* aPlan, string actionName, double currentTime, double newTime)  //,include double current time
    {
        ...
        for (pc_list<plan_step*>::const_iterator i = aPlan->begin(); i != aPlan->end(); ++i) {  //  cout <<  getName(*i) << " == " << actionName <<" && "<< (*i)->start_time <<" == "<<currentTime<<"\n";
            if (getName(*i) == actionName && (*i)->start_time == currentTime) {
    ...
    void PlanRepair::repairPlan()
    {
        ...
        if (firstPlanStep != 0) {
            ...
            if (!planRepaired)
                planRepaired = shakePlan(repairingPlan, firstPlanStep, int(deadLine / 2));  //need to set the variation somehow, half the deadLine time seems to work pretty good
        };
    ...
    bool PlanRepair::slideEndOfPlan(const plan* repairingPlan, const plan_step* firstAction)
    {
        ...
        while (searching) {
            ...
            if (counter >= 8191 || ((shiftTime - lastShiftTime) < v.getTolerance() && (shiftTime - lastShiftTime) > 0))
                searching = false;
