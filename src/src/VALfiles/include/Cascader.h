...
class CascadeSet
{
  protected:
    ...
    template <class TI>
    CascadeSet(TI s, TI e)
        : terminal(false), cascade()
    {
        insert(s, e);
    };
  ...
  public:
                     ...
                     <U*, size_t>
    {
      ...
      public:
        ...
        {
            return cmap == cmi.cmap && done == cmi.done && c == cmi.c && (cmpi ? (cmi.cmpi ? (*cmpi == *cmi.cmpi) : false) : true);
        };
        bool operator!=(const CascadeMap<T, U>::iterator& cmi) const
        {
            return !(operator==(cmi));
