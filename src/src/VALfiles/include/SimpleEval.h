...
namespace Inst
{
    ...
    class SimpleEvaluator : public VAL::VisitController
    {
      ...
      public:
        ...
        SimpleEvaluator(VAL::TypeChecker* const tcIn, VAL::FastEnvironment* const ff, PrimitiveEvaluatorConstructor<PEC> pec)
            : valueTrue(true), unknownTrue(false), valueFalse(false), unknownFalse(false), tc(tcIn), f(ff),
