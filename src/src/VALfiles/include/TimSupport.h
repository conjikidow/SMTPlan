...
namespace TIM
{
    ...
    class Property
    {
      ...
      public:
        Property()
            : predicate(0), isSV(false), isReq(false){};
...
#define cTPS(x) const_cast<TIMpredSymbol*>(static_cast<const TIMpredSymbol*>(x))
    ...
    class TIMobjectSymbol : public VAL::const_symbol
    {
      ...
      public:
        TIMobjectSymbol(const string& s)
            : const_symbol(s){};
...
#define TOB(x) static_cast<TIM::TIMobjectSymbol*>(x)
    ...
    class PropertyState
    {
      private:
        ...
        template <class TI>
        PropertyState(TIMAnalyser* t, TI s, TI e)
            : tan(t), properties(s, e){};
    ...
    struct recordSV
    {
        ...
        recordSV(PropertySpace* p, vector<Property*>& s)
            : ps(p), sv(s){};
    ...
    class TransitionRule
    {
      ...
      public:
        ...
        void write(ostream& o) const
        {
            o << (*enablers) << " => " << (*lhs) << " -> " << (*rhs) << (isAttribute() ? " attribute rule: " : "") << (isIncreasing() ? "increasing" : "") << (isDecreasing() ? "decreasing" : "");
        };
    ...
    class DurativeActionPredicateBuilder : public VAL::VisitController
    {
      ...
      public:
        DurativeActionPredicateBuilder()
            : VisitController(), inserting(true){};
...
#define TASc(x) static_cast<const TIM::TIMactionSymbol* const>(x)
    ...
    struct mRec
    {
        ...
        mRec(Property* x, int y, opType z)
            : first(x), second(y), opt(z){};
 ...
 */
    ...
    struct mutRec
    {
        ...
        bool operator<(const mutRec& m) const
        {
            return (first < m.first || (first == m.first && (second < m.second || (second == m.second && (one < m.one || (one == m.one && other < m.other))))));
        };
    };
