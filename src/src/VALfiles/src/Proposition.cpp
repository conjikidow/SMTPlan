...
namespace VAL
{
    ...
    pair<intervalEnd, intervalEnd> getIntervalFromPt(intervalEnd p, const vector<pair<intervalEnd, intervalEnd>>& ints, const pair<intervalEnd, intervalEnd>& int1)
    {
        ...
        for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i = someIntervals.begin(); i != someIntervals.end(); ++i) {
            if ((p.first >= i->first.first) && (p.first <= i->second.first)) {
                ...
                if (!(((p.first == i->first.first) && (!(i->first.second)) && (p.second)) || ((p.first == i->second.first) && (!(i->second.second)) && (p.second)) ||
    ...
    Intervals setUnion(const Intervals& ints1, const Intervals& ints2)
    {
        ...
        for (unsigned int count = 0; count != theIntervals.size(); ++count) {
            ...
            for (unsigned int count2 = 0; count2 != theIntervals.size() + 1; ++count2) {
                for (vector<pair<intervalEnd, intervalEnd>>::iterator i = theIntervals.begin(); i != theIntervals.end(); ++i) {
                    if (((isPointInInterval(i->first.first, aInterval) && !((i->first.first == aInterval.second.first) && !(i->first.second) && !(aInterval.second.second))) || ((i->first.first == aInterval.second.first) && (i->first.second || aInterval.second.second)))
            ...
            for (vector<pair<intervalEnd, intervalEnd>>::iterator i = theIntervals.begin(); i != theIntervals.end(); ++i) {
                if ((i->first.first >= progress) && (i->first.first <= startPt.first)) {
                    if (!((i->first.first == aInterval.first.first) && (i->second.first == aInterval.second.first)))  //check not adding a single point again
                    {
                        ...
                        if (!((tempInterval.first.first == 0) && (tempInterval.second.first == 0) && (!(tempInterval.first.second)) && (!(tempInterval.second.second)))) {
                            startPt.first = i->first.first;
                            if (!(i->first.second))
    ...
    void Comparison::write(ostream& o) const
    {
        ...
        o << "(" << getExprnString(comp->getLHS(), bindings) << " " << op << "  " << getExprnString(comp->getRHS(), bindings) << ")";
    };
    ...
    ConjGoal::evaluate(const State* s, vector<const DerivedGoal*> DPs) const
    {
        ...
        for (vector<const Proposition*>::const_iterator i1 = conditions.begin(); i1 != conditions.end(); ++i1) {  //cout << **i1 <<" conj\n";
    ...
    {  // cout << gs.size() << " disj size\n";
        ...
        if (rnk.second == 0) {
            for (vector<const Proposition*>::const_iterator i = gs.begin(); i != gs.end(); ++i) {  // cout << **i <<" disj\n";
                if ((*i)->evaluate(s, DPs))
                    return true;
            };
...
*/
   ...
   return false;
    ...
    {
        return o.markOwnedPreconditionFEs(a, comp->getLHS(), bindings) && o.markOwnedPreconditionFEs(a, comp->getRHS(), bindings);
    };
    ...
    Intervals DerivedGoal::getIntervals(const State* s) const
    {
        ...
        if ((j->first.first == 0) && (j->first.second == true) && (j->second.first == endOfInterval) && (j->second.second == true))
    ...
    bool isExpressionConstant(const expression* e, const ActiveCtsEffects* ace, const Environment& bs, CoScalar endInt)
    {
        ...
        if (dynamic_cast<const minus_expression*>(e)) {
            return isExpressionConstant(dynamic_cast<const minus_expression*>(e)->getLHS(), ace, bs, endInt) && isExpressionConstant(dynamic_cast<const minus_expression*>(e)->getRHS(), ace, bs, endInt);
        };

        if (dynamic_cast<const mul_expression*>(e)) {
            return isExpressionConstant(dynamic_cast<const mul_expression*>(e)->getLHS(), ace, bs, endInt) && isExpressionConstant(dynamic_cast<const mul_expression*>(e)->getRHS(), ace, bs, endInt);
        };

        if (dynamic_cast<const plus_expression*>(e)) {
            return isExpressionConstant(dynamic_cast<const plus_expression*>(e)->getLHS(), ace, bs, endInt) && isExpressionConstant(dynamic_cast<const plus_expression*>(e)->getRHS(), ace, bs, endInt);
        };
    ...
    void Comparison::setUpComparisons(const ActiveCtsEffects* ace, bool rhsOpen)
    {
        ...
        if (const func_term* fexpression = dynamic_cast<const func_term*>(comp->getLHS())) {
            ...
            if (i != ace->activeFEs.end()) {  //cout << " cts ftn = "<<  *(i->second->ctsFtn) <<"\n";
	...
	*/
    ...
    string Comparison::getExprnString(const expression* e, const Environment& bs) const
    {
        if (dynamic_cast<const div_expression*>(e)) {
            return "(" + getExprnString(dynamic_cast<const div_expression*>(e)->getLHS(), bs) + " / " + getExprnString(dynamic_cast<const div_expression*>(e)->getRHS(), bs) + ")";
        };

        if (dynamic_cast<const minus_expression*>(e)) {
            return "(" + getExprnString(dynamic_cast<const minus_expression*>(e)->getLHS(), bs) + " - " + getExprnString(dynamic_cast<const minus_expression*>(e)->getRHS(), bs) + ")";
        };

        if (dynamic_cast<const mul_expression*>(e)) {
            if (LaTeX)
                return "(" + getExprnString(dynamic_cast<const mul_expression*>(e)->getLHS(), bs) + " $\\times$ " + getExprnString(dynamic_cast<const mul_expression*>(e)->getRHS(), bs) + ")";

            return "(" + getExprnString(dynamic_cast<const mul_expression*>(e)->getLHS(), bs) + " * " + getExprnString(dynamic_cast<const mul_expression*>(e)->getRHS(), bs) + ")";
        };

        if (dynamic_cast<const plus_expression*>(e)) {
            return "(" + getExprnString(dynamic_cast<const plus_expression*>(e)->getLHS(), bs) + " + " + getExprnString(dynamic_cast<const plus_expression*>(e)->getRHS(), bs) + ")";
        };
    ...
    {
        if (dynamic_cast<const div_expression*>(e)) {
            return "(" + getExprnString(dynamic_cast<const div_expression*>(e)->getLHS(), bs, s) + " / " + getExprnString(dynamic_cast<const div_expression*>(e)->getRHS(), bs, s) + ")";
        };

        if (dynamic_cast<const minus_expression*>(e)) {

            return "(" + getExprnString(dynamic_cast<const minus_expression*>(e)->getLHS(), bs, s) + " - " + getExprnString(dynamic_cast<const minus_expression*>(e)->getRHS(), bs, s) + ")";
        };

        if (dynamic_cast<const mul_expression*>(e)) {
            if (LaTeX)
                return "(" + getExprnString(dynamic_cast<const mul_expression*>(e)->getLHS(), bs, s) + " $\\times$ " + getExprnString(dynamic_cast<const mul_expression*>(e)->getRHS(), bs, s) + ")";

            return "(" + getExprnString(dynamic_cast<const mul_expression*>(e)->getLHS(), bs, s) + " * " + getExprnString(dynamic_cast<const mul_expression*>(e)->getRHS(), bs, s) + ")";
        };

        if (dynamic_cast<const plus_expression*>(e)) {
            return "(" + getExprnString(dynamic_cast<const plus_expression*>(e)->getLHS(), bs, s) + " + " + getExprnString(dynamic_cast<const plus_expression*>(e)->getRHS(), bs, s) + ")";
        };
        ...
        *report << "(Follow each of:\n";
        ...
        for (vector<const AdviceProposition*>::const_iterator i = adviceProps.begin(); i != adviceProps.end(); ++i) {
...

            {
                ...
                *report << "and ";
                if (dynamic_cast<const AdvicePropositionConj*>(*i) || dynamic_cast<const AdvicePropositionDisj*>(*i)) {
                    *report << "\n";
                    displayIndent(indent + 4);
                };
            } else
