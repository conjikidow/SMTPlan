...
namespace TIM
{
    ...
    struct recordObjectIn
    {
        PropertySpace* ps;
        bool added;
        recordObjectIn(PropertySpace* p)
            : ps(p), added(false){};
    ...
    struct extendWithIncrRule
    {
        PropertySpace* ps;
        bool extended;
        extendWithIncrRule(PropertySpace* p)
            : ps(p), extended(false){};
    ...
    struct notMatchFor
    {
        ...
        notMatchFor(pddl_type* p, Property* pr)
            : pt(p), prop(pr){};
    ...
    Property* Property::getBaseProperty(const pddl_type* pt) const
    {
        ...
        TIMpredSymbol* tps =
            static_cast<TIMpredSymbol*>(EPS(predicate)->getParent()->find(predicate, makeTT(EPS(predicate)->tBegin(), posn, pt),
    ...
    struct aMxs2
    {
        ...
        aMxs2(PropertySpace* tps, Property* tp)
            : ps(tps), p(tp){};
...
*/
    ...
    void mutex::write(ostream& o) const
    {
        for (set<mutRec>::const_iterator i = argPairs.begin(); i != argPairs.end(); ++i) {
            if (op1 == op2) {
                ...
                o << "Mutex for '" << op1->name->getName() << "' and '" << op2->name->getName() << " args: " << i->first << " " << i->second << "\n";
    ...
    struct addMx1
    {
        ...
        addMx1(operator_* o, const mRec& p)
            : op(o), pr(p){};
    ...
    {
        if (find(lhs->begin(), lhs->end(), p) == lhs->end() && find(rhs->begin(), rhs->end(), p) == rhs->end()) {
            return 0;
        };
