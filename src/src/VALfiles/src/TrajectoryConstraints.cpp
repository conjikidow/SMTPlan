...
#define MESSAGE(x)                \
    ...
    class TrajectoryConstraintsMonitor::CollectProps : public VisitController
    {
      ...
      public:
        ...
        void visit_constraint_goal(constraint_goal* cg)
        {
            ...
            const Proposition* p = !e1 ? tjm.vld->pf.buildProposition(cg->getRequirement()) : tjm.vld->pf.buildProposition(cg->getRequirement(), *e1, true, 0);
            e1 = bs.empty() ? 0 : bs.copy(tjm.vld);
            const Proposition* q = cg->getTrigger() ? (!e1 ? tjm.vld->pf.buildProposition(cg->getTrigger()) : tjm.vld->pf.buildProposition(cg->getTrigger(), *e1, true, 0)) : 0;
	...
	else if(Verbose) 
	{
        ...
        for (PropositionPairs::const_iterator i = sometimeAfter.begin(); i != sometimeAfter.end(); ++i) {
            if ((i->first)->evaluate(&s)) {
                MESSAGE("Triggered a requirement for " << *(i->second) << " after " << *(i->first) << " at " << s.getTime());
                PropMonitor pm(i->second);
                passOn(pm, *i);
